...
# Time and Date for options 3 and 4
...
if [[ "$layout" == 'NO' ]]; then
	...
	option_2=" $status"
	option_3="⏰ $current_time"
	option_4=" $current_date"
else
...
run_cmd() {
	...
	if [[ "$1" == '--opt1' ]]; then
		...
		notify-send -u low "⏰ Current Time: $current_time"
	elif [[ "$1" == '--opt4' ]]; then
		notify-send -u low " Date: $current_date"
	fi
}
